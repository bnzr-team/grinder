#!/usr/bin/env bash
# ops_connector_triage.sh -- One-command connector triage wrapper (Launch-10 PR3).
#
# Single entrypoint for connector evidence scripts:
#   market-data    -> fire_drill_connector_market_data.sh
#   exchange-port  -> fire_drill_connector_exchange_port.sh
#
...
#
# Modes:
#   market-data    L2 parse, DQ staleness/gaps/outliers, symbol whitelist (~2s).
#   exchange-port  Gate chain, idempotency, retry classification (~2s).
#
# Exit codes:
#   0  All checks passed
...
print_usage() {
  cat <<'USAGE'
Usage: bash scripts/ops_connector_triage.sh <mode> [options]

Modes:
  market-data    L2 parse validation, DQ staleness/gaps/outliers, symbol
                 whitelist filtering (21 assertions, ~2s)
  exchange-port  Gate chain (NOT_ARMED, kill-switch, drawdown, whitelist),
                 idempotency cache, retry classification (40 assertions, ~2s)
...
Examples:
  bash scripts/ops_connector_triage.sh market-data
  bash scripts/ops_connector_triage.sh exchange-port
  bash scripts/ops_connector_triage.sh exchange-port --no-status
...
if [[ -z "$MODE" ]]; then
  die "No mode specified. Usage: bash scripts/ops_connector_triage.sh <mode> (try -h for help)"
fi
...
case "$MODE" in
  ...
  exchange-port)
    ...
    ;;
  *)
    die "Unknown mode: '$MODE'. Valid modes: market-data, exchange-port (try -h for help)"
    ;;
esac
...
case "$MODE" in
  market-data)
    ...
    echo "  - Exchange port drill: bash scripts/ops_connector_triage.sh exchange-port"
    echo "  - Full triage: $TRIAGE_DOC"
    ;;
  exchange-port)
    echo "  - Paste summary.txt + sha256sums.txt into PR body or incident notes"
    echo "  - Market data drill: bash scripts/ops_connector_triage.sh market-data"
    echo "  - Full triage: $TRIAGE_DOC"
    ;;
esac
