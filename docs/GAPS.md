...
| Data quality (GapDetector, outlier filtering) | `docs/02_DATA_SOURCES.md` | `src/grinder/data/quality.py`, `src/grinder/data/quality_metrics.py`, `src/grinder/data/quality_engine.py` | **PARTIAL** | **P1** | Data | PR1: detect-only classes + metrics. PR2: wired into LiveFeed. PR3: dq_blocking gate + 3 block reasons in remediation (dq_blocking=False default). Remaining: alerts | Alerts on DQ events |
