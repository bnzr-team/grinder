...
| Consecutive loss limit | `docs/10_RISK_SPEC.md` | `src/grinder/risk/consecutive_loss_guard.py`, `src/grinder/risk/consecutive_loss_wiring.py` | **PARTIAL** | **P2** | Risk | PR-C3: pure logic library (24 tests). PR-C3b: live wiring (19 tests). PR-C3c: per-symbol guards + persistent state (16 new tests). Metrics: count=max across symbols, trips=sum. | DEGRADED action (needs FSM input channel); RoundtripTracker persistence; FillModelV0 integration |
