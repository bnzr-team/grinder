...
| Latency / retry (LatencyMonitor, OrderRetryPolicy) | `docs/09_EXECUTION_SPEC.md` §9.8-9.9 | `src/grinder/net/`, `src/grinder/observability/latency_metrics.py` | **PARTIAL** | **P1** | Execution | PR1: HttpRetryPolicy, DeadlinePolicy, MeasuredHttpClient, HTTP metrics (counters + histogram). Safe-by-default (retries disabled). PR2: wire into exchange paths behind LATENCY_RETRY_ENABLED flag. | PR2: wiring into Binance REST calls; smoke confirms latency_ms histogram populated |
