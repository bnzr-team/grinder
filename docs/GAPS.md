...
| Consecutive loss limit | `docs/10_RISK_SPEC.md` | `src/grinder/risk/consecutive_loss_guard.py`, `src/grinder/risk/consecutive_loss_wiring.py` | **PARTIAL** | **P2** | Risk | PR-C3: pure logic library (24 tests). PR-C3b: live wiring shipped (ConsecutiveLossService in `scripts/run_live_reconcile.py` only). PAUSE-only action, 2 metrics, 2 alerts, 19 tests. | Per-symbol tracking (v1 = global streak); persistent state across restarts; DEGRADED action (needs FSM input channel) |
