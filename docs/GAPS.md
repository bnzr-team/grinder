...
| Production trading loop (DataConnector → LiveEngineV0) | `docs/03_ARCHITECTURE.md` §3.6 | `scripts/run_trading.py` | **PARTIAL** | **P2** | Core | PR-P2-LOOP-0: `run_trading.py` wires LiveConnectorV0 → LiveEngineV0.process_snapshot(). Modes: read_only/paper/live_trade (affect gating only; NoOpExchangePort — no real execution). HA gating not implemented. Safety: ACK required for paper/live_trade. `grinder_live_engine_initialized` = 1 after startup. Fixture mode (JSONL). | Real ExchangePort for live_trade; HA-gated trading (ACTIVE-only); Runbook 32 ceremony end-to-end |
