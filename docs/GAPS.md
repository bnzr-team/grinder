...
| Data quality (GapDetector, outlier filtering) | `docs/02_DATA_SOURCES.md` | `src/grinder/data/quality.py`, `src/grinder/data/quality_metrics.py`, `src/grinder/data/quality_engine.py` | **PARTIAL** | **P1** | Data | PR1: detect-only classes + metrics. PR2: wired into LiveFeed via DataQualityEngine (dq_enabled gated, metrics-only). Remaining: block reasons + gating (PR3) | Block reasons in remediation; can_execute() DQ gate; alerts |
