...
| Fill probability model (`estimate_fill_probability()`) | `docs/09_EXECUTION_SPEC.md` §9.4 | `src/grinder/ml/fill_model_v0.py` | **PARTIAL** | **P2** | Research | PR-C2: FillModelV0 baseline (pure Python, bin-count calibration, shadow-only). Not wired to execution. | Wire to execution as level filter; online features; drift detection |
...
| Consecutive loss limit | `docs/10_RISK_SPEC.md` | `src/grinder/risk/consecutive_loss_guard.py`, `src/grinder/risk/consecutive_loss_wiring.py` | **PARTIAL** | **P2** | Risk | PR-C3: pure logic library (24 tests). PR-C3b: live wiring (19 tests). PR-C3c: per-symbol guards + persistent state (16 new tests). PR-C3d: RoundtripTracker persistence + restart recovery (6 new tests). Metrics: count=max across symbols, trips=sum. | DEGRADED action (needs FSM input channel); FillModelV0 integration |
...
| Production trading loop (DataConnector → LiveEngineV0) | `docs/03_ARCHITECTURE.md` §3.6 | `scripts/run_live.py` | **PLANNED** | **P2** | Core | `run_live.py` is observability shell only (HTTP + HA + probe); no code path instantiates `LiveEngineV0` or calls `process_snapshot()`. Runbook 32 phases beyond Phase 0/1 blocked until trading loop exists. `grinder_live_engine_initialized` gauge (PR-C4) = 0 confirms engine not running. | Entrypoint that wires DataConnector → LiveFeed → LiveEngineV0.process_snapshot(); `grinder_live_engine_initialized` = 1 in prod; Runbook 32 ceremony executable end-to-end |
