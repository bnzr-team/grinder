...
| Consecutive loss limit | `docs/10_RISK_SPEC.md` | `src/grinder/risk/consecutive_loss_guard.py` | **PARTIAL** | **P2** | Risk | PR-C3: ConsecutiveLossGuard v1 (pure logic, 24 tests, 2 alert rules). Not wired to live pipeline. | Wire to live pipeline (RoundtripTracker → guard); per-symbol tracking; persistent state |
