...
| Consecutive loss limit | `docs/10_RISK_SPEC.md` | `src/grinder/risk/consecutive_loss_guard.py` | **PARTIAL** | **P2** | Risk | PR-C3: ConsecutiveLossGuard v1 (pure logic library, 24 tests). No live wiring, no metrics, no alerts yet. | PR-C3b: wire to live pipeline (RoundtripTracker → guard → metrics → alerts → operator_override); per-symbol tracking; persistent state |
