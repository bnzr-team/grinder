...
| Fill tracking (FillTracker, RoundTrip, PositionSyncer) | `docs/09_EXECUTION_SPEC.md` §9.6-9.7 | `src/grinder/execution/` | **PARTIAL** | **P1** | Execution | Launch-06 PR1: FillTracker MVP + FillMetrics + RB26 (detect-only); wiring + PositionSyncer pending | `FillTracker` + `PositionSyncer` minimal: reconcile fills/positions; tests; metrics: open positions, sync lag |
